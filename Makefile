...
test:
	...
	curl -X POST -F "data=@input_eurostar.txt" -F "tokenizer=" -F "tagger=" -F "parser=" "http://localhost:9090/process" | PYTHONIOENCODING=utf-8 python -c "import sys,json; sys.stdout.write(json.load(sys.stdin)['result'])"
